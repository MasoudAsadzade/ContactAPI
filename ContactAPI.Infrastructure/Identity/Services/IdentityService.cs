...
using ContactAPI.Application.DTOs.Settings;
using ContactAPI.Application.Enums;
using ContactAPI.Application.Exceptions;
...
namespace ContactAPI.Infrastructure.Identity.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<Result<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //await _mailService.SendAsync(new MailRequest() { From = "Masoud.Asadzade@gmail.com", To = user.Email, Body = $"Please confirm your account by <a href='{verificationUri}'>clicking here</a>.", Subject = "Confirm Registration" });
                    return Result<string>.Success(user.Id, message: $"User Registered. Confirmation Mail has been delivered to your Mailbox. " +
                        $"Already Confirmed! Please Log In by Generating a JSON Web Token for a valid combination of your email= {user.Email} and password.");
                }
                else
                {
        ...
        private async Task<string> SendVerificationEmail(ApplicationUser user, string origin)
        {
            ...
            //To avoid Verifing Email
            await ConfirmEmailAsync(user.Id, code);
